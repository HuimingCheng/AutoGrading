...
def recog_name(image):
    return image_to_string(image, config="-c tessedit_char_whitelist=-ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz -psm 6").strip()
...
def image_proc(filename,n_pos):
    ...
    image = image.crop((left, upper, image.size[1], lower))
    # image.show()
