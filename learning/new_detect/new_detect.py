...
if __name__ == '__main__':
	...
	cv2.imwrite("Whatever.jpg",crop_img)
	# cv2.imwrite("crop",crop_img)
	crop_img = cv2.resize(crop_img,None,fx=2, fy=2, interpolation = cv2.INTER_LANCZOS4)
	...
	# print len(cnts_rect)
	cv2.drawContours(crop_img, cnts_rect, -1, (0, 255, 0), 10)
	cv2.imshow("Draw",crop_img)
	cv2.imwrite("Green.jpg",crop_img)
	# x_axis is a list, store all the x_axis data of one contour
	# y_axis is a list, store all the y_axis data of same contour
	# cnts[0] is a list of point, which is one rectangle
